...
describe('WorkspaceBrowser.module.css list', () => {
  ...
  it('renders expanded categories as light nested containers without guide rails or row movement', () => {
    const group = declarations('.categoryGroup')
    const expanded = declarations('.categoryGroupExpanded')
    const guide = declarations('.categoryGroupExpanded::before')
    const nestedSession = rowDeclarations('.categorizedSessionRow')

    expect(group?.get('margin-top')).toBe('2px')
    expect(group?.get('padding-top')).toBeUndefined()
    expect(group?.get('padding-bottom')).toBeUndefined()
    expect(expanded?.get('padding-top')).toBe('2px')
    expect(expanded?.get('padding-bottom')).toBe('2px')
    expect(expanded?.get('background')).toContain('color-mix')
    expect(guide).toBeUndefined()
    expect(nestedSession?.get('padding-left')).toBe('calc(8px + var(--category-depth, 0) * 18px)')
    expect(rowDeclarations('.categoryRow.categoryDropTarget')?.get('box-shadow')).toContain('inset')
  })
