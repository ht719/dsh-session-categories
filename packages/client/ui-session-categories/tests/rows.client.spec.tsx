...
describe('workspace browser rows', () => {
  ...
  it('shows a collapsed workspace indicator for hidden categories and sessions', () => {
    const group: GroupNode = {
      key: 'project', workspaceId: wid('project'), cwd: '/projects/project', createdAt: 0, label: 'Project',
    ...
    render(<ProjectRowItem group={group} onToggle={vi.fn()} onCreate={vi.fn()}
      collapsedIndicator="包含分类和会话" t={t} />)
    expect(screen.getByTitle('包含分类和会话')).toBeTruthy()
    expect(screen.queryByText('2 个分类 · 3 个会话')).toBeNull()
  })
