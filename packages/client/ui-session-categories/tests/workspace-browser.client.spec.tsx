...
function mount(overrides: Partial<WorkspaceBrowserProps> = {}) {
  ...
  const props: WorkspaceBrowserProps = {
    ...
    useCategories: hook({ status: 'ready', snapshots: {}, error: null }),
    ensureCategories: vi.fn(async () => ({ ok: true })),
    createCategory: vi.fn(async () => ({ ok: true })),
    renameCategory: vi.fn(async () => ({ ok: true })),
    moveCategory: vi.fn(async () => ({ ok: true })),
    reorderCategory: vi.fn(async () => ({ ok: true })),
    assignSessions: vi.fn(async () => ({ ok: true })),
...
describe('WorkspaceBrowser', () => {
  ...
  it('rail add-workspace opens the direct native chooser with no menu or expansion', async () => {
    const expandSidebar = vi.fn()
    const pickDirectory = vi.fn(async () => null)
    mount({ wide: false, expandSidebar, pickDirectory, useWorkspaces: hook(workspaceState([workspace('alpha', [])])) })
  ...
  it('always offers add because the direct native chooser ships with the browser', () => {
